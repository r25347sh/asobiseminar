/**
 * MENU/MENU.js — Fluid Fibonacci Arc implementation for asobiseminar (restored)
 */
...
(function () {
  ...
  function computeSafeArc(cx, cy, itemCount){
    ...
    if ((left<nearCornerThreshold&&top<nearCornerThreshold)||(left<nearCornerThreshold&&bottom<nearCornerThreshold)||(right<nearCornerThreshold&&top<nearCornerThreshold)||(right<nearCornerThreshol[...]
  }
  ...
  function computeSpiralPositions(n, startAngle, endAngle, inner, outer){
    ...
    for(let i=0;i<n;i++){ const t = n===1?0.98:i/(n-1); const theta = startAngle + t*span + rotationOffset; const r = inner + (outer-inner)*((Math.exp(c*t)-1)/(Math.exp(c)-1)); const sizeScale = 0[...]
    return positions;
  }
  ...
  function createMenuDOM(){
    ...
    coreBtn = document.createElement('button'); coreBtn.className='rm-core-btn fib-core'; coreBtn.type='button'; coreBtn.innerHTML='✕'; coreBtn.setAttribute('aria-label','戻る'); coreBtn.addEventListener('click',(e)=>{ e.stopPropagation(); if(menuStack.length>0){ renderMenuLevel(menuStack.pop()); triggerParticleBurst(); } else closeMenu(); }); document.body.appendChild(menuWrapper);
  }

  function renderMenuLevel(items){
    currentItems = items; itemsContainer.querySelectorAll('.rm-item').forEach(el=>{ el.classList.remove('rendered'); setTimeout(()=>el.remove(),220); }); orbitsContainer.innerHTML=''; const safe = computeSafeArc(openX, openY, items.length); const startAngle = safe.startAngle, endAngle = safe.endAngle; const outer = clamp(Math.min(window.innerWidth, window.innerHeight)*0.36,[...]
  }

  function openMenu(x,y){ if(!menuWrapper) createMenuDOM(); openX=x; openY=y; const margin=24; const left=clamp(x, margin, window.innerWidth-margin); const top=clamp(y, margin, window.innerHeight-[...]
  function closeMenu(){ if(!menuWrapper) return; menuWrapper.classList.remove('active'); menuWrapper.classList.remove('open'); if(scrim) scrim.classList.remove('active'); itemsContainer.querySelec[...]

  function angleBetween(cx,cy,px,py){ return Math.atan2(py-cy, px-cx); }

  function onMenuPointerDown(e){ if(!isOpen) return; const id=e.pointerId; const cx=openX, cy=openY; pointerTracking={id, startAngle:angleBetween(cx,cy,e.clientX,e.clientY), startRot:rotationOffse[...]

  function onMenuPointerMove(e){ if(!pointerTracking || e.pointerId!==pointerTracking.id) return; const cx=openX, cy=openY; const a=angleBetween(cx,cy,e.clientX,e.clientY); const delta = a - point[...]

  function findNearestItemToPoint(px,py){ const items = Array.from(itemsContainer.children); let best=null, bestDist=Infinity, bestIdx=-1; items.forEach((el,i)=>{ const rect=el.getBoundingClientRe[...]

  function onMenuPointerUp(e){ if(!pointerTracking || e.pointerId!==pointerTracking.id) return; try{ e.target.releasePointerCapture && e.target.releasePointerCapture(e.pointerId); }catch(_){ } con[...]

  function initOpenGestures(){ let sx=0, sy=0, timer=null, tapCount=0, tapTimer=null; document.addEventListener('pointerdown', (e)=>{ if(isOpen && menuWrapper && menuWrapper.contains(e.target)) r[...]
  function initMenuPointerHandlers(){ document.addEventListener('pointerdown', (e)=>{ if(!isOpen) return; if(menuWrapper && menuWrapper.contains(e.target)){ onMenuPointerDown(e); } }); document.a[...]

  function boot(){ createMenuDOM(); initOpenGestures(); initMenuPointerHandlers(); document.addEventListener('keydown', (e)=> { if(e.key==='Escape' && isOpen) closeMenu(); }); document.addEventLi[...]
