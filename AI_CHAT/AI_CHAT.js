 ...
 * Style presets apply instantly to the page (injectCSS). Optional WebLLM.
 */
(function () {
  'use strict';
  ...
  var SYSTEM_PROMPT = [
    'You style the Asobi Lab website. Reply ONLY JSON:',
    '{"reply":"短い日本語","actions":[{"op":"injectCSS","css":"body{...}"}]}',
    'Never refuse. Prefer injectCSS for theme changes.'
  ].join('\n');
  ...
  function tryLocalStylePreset(userText) {
    var t = userText.trim();
    if (/中華|中国風|チャイニーズ|中華風/.test(t))
      return { reply: '中華風デザインにしました。', actions: [{ op: 'injectCSS', css: 'body{font-family:"Noto Serif JP","Songti SC",serif!important;background:#1a0a0a!important;color:#f5e6c8!important}h1,h2,.section-title{color:#ffd700!important;border-bottom:2px solid #c41e3a!important;text-shadow:0 0 12px rgba(196,30,58,.4)!important}.card,.card-group{background:#2a1212!important;border:1px solid #c41e3a!important;color:#f5e6c8!important}.hero,.hero-v2{background:linear-gradient(135deg,#1a0a0a,#3d1515)!important}.btn-play{background:#c41e3a!important;color:#fff!important}a{color:#ffd700!important}.site-header,.site-footer{background:#0d0505!important;color:#f5e6c8!important}' }] };
    if (/洋風|ヨーロピアン|クラシック|欧風/.test(t))
      return { reply: '洋風トーンにしました。', actions: [{ op: 'injectCSS', css: 'body{font-family:Georgia,"Times New Roman",serif!important;background:#f5f0e8!important;color:#2c2c2c!important}h1,h2,.section-title{font-family:Georgia,serif!important;color:#1a365d!important}.card,.card-group{background:#fff!important;border:1px solid #c9b896!important}.hero,.hero-v2{background:linear-gradient(135deg,#f8f4ec,#ebe3d5)!important}.btn-play{background:#1a365d!important}' }] };
    if (/和風|日本風/.test(t))
    ...
    if (/ネオン|サイバー|未来/.test(t))
      return { reply: 'ネオン調にしました。', actions: [{ op: 'injectCSS', css: 'body{background:#0a0a12!important;color:#e0f7ff!important}h1,h2,.section-title{color:#00f0ff!important;text-shadow:0 0 16px #00f0ff!important}.card,.card-group{background:#12121f!important;border:1px solid #7c3aed!important}.btn-play{background:linear-gradient(90deg,#7c3aed,#00f0ff)!important}' }] };
    if (/パステル|可愛|かわいい|ソフト/.test(t))
      return { reply: 'パステル調にしました。', actions: [{ op: 'injectCSS', css: 'body{background:#fdf2f8!important;color:#4a1942!important}h1,h2,.section-title{color:#db2777!important}.card,.card-group{background:#fff!important;border:1px solid #f9a8d4!important}.hero,.hero-v2{background:linear-gradient(135deg,#fce7f3,#e0e7ff)!important}.btn-play{background:#ec4899!important}' }] };
  ...
  function createUI() {
    ...
    panel.innerHTML = [
      ...
      '    <textarea class="ai-chat-input" id="ai-input" rows="1" placeholder="例: 中華風にして / 洋風にして"></textarea>',
      '    <button type="button" class="ai-btn ai-btn-send" id="ai-btn-send">➤</button>',
      '  </div>',
      '  <div class="ai-chat-hints">',
      '    <button type="button" data-hint="中華風のデザインにして">中華風</button>',
      '    <button type="button" data-hint="デザインを和風にして">和風</button>',
      '    <button type="button" data-hint="洋風にして">洋風</button>',
    ...
    appendMessage('system', '「中華風」「和風」「洋風」「ダーク」などはボタンまたは入力で即反映されます。');
  }
  ...
  async function initEngine() {
    ...
    try {
      ...
      appendMessage('system', 'LLM読込失敗でもプリセット（中華風/和風等）は使えます。');
    }
  }
  ...
  async function runChat(userText) {
    ...
    if (preset) {
      ...
      badge.textContent = '✓ ページに適用・保存';
      el.appendChild(badge);
      return;
    }

    if (!engineReady || !engine) {
      appendMessage('assistant', 'モデル準備中です。中華風・和風・洋風・ダークは今すぐ使えます。');
    ...
    try {
      var chunks = await engine.chat.completions.create({
        messages: [
          ...
          { role: 'user', content: userText + '\nJSON only with injectCSS actions.' }
        ],
        temperature: 0.15,
        max_tokens: 500,
        stream: true
      ...
      if (parsed && Array.isArray(parsed.actions) && parsed.actions.length) {
        ...
        b.textContent = '✓ ' + parsed.actions.length + ' 件をページに適用';
        msgEl.appendChild(b);
      } else {
        var again = tryLocalStylePreset(userText);
        if (again) {
          applyActions(again.actions, true);
          msgEl.textContent = again.reply;
          var ab = document.createElement('div');
          ab.className = 'actions-badge';
          ab.textContent = '✓ ページに適用・保存';
          msgEl.appendChild(ab);
        } else {
          msgEl.textContent = (full || '解析失敗') + '\n※「中華風にして」のように具体的に依頼するか、下のボタンを使ってください。';
        }
      }
    } catch (err) {
