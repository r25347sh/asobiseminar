(function () {
  ...
  function applyEditorPermissions() {
    ...
    if ($('sel-info')) {
      if (simple) {
        $('sel-info').textContent = '本文をクリックして Googleドキュメント風に編集。画像・音楽・動画は右のマイファイルから挿入。';
      } else {
  ...
  function pickTarget(el) {
    ...
    /* シンプル編集: 既存セクション／コンテンツブロック単位を1つのテキストエリアとして扱う */
    if (isSimpleEditor()) {
      var block = el.closest && el.closest(
        'section.article_by_teacher, section.content-block, section.goal-banner, section.info-card, main .content-block, [data-cms-editable="true"]'
      );
      if (block && !isLocked(block)) return block;
      /* ロックされていない section 全体 */
      var sec = el.closest && el.closest('section, article, .card');
      if (sec && !isLocked(sec) && sec !== doc().body) return sec;
      /* フォールバック: テキスト要素 */
      var curS = el;
      var TEXTISH_S = { P:1, H1:1, H2:1, H3:1, H4:1, H5:1, H6:1, SPAN:1, A:1, LI:1, BLOCKQUOTE:1, STRONG:1, EM:1, B:1, I:1, UL:1, OL:1 };
      while (curS && curS !== doc().body) {
        if (isLocked(curS)) return null;
        if (TEXTISH_S[curS.tagName]) return curS;
        curS = curS.parentElement;
      }
      return null;
    }
  ...
  function enterEdit(el) {
    ...
    /* Googleドキュメント風: 改行で段落が作られるよう */
    try { el.style.minHeight = el.style.minHeight || '1.5em'; } catch (e0) {}
    state.selected = el;
    ...
    if ($('sel-info')) {
      if (isSimpleEditor()) {
        $('sel-info').textContent = '本文編集中（Googleドキュメント風）— 改行OK / 太字・リストは上部バー / 画像・動画は右のマイファイル';
      } else {
    ...
    getFile(path).then(function (f) {
      ...
      if (isHtml) {
        ...
        fEl.onload = function () {
          ...
          setupFrameEvents();
          status(isSimpleEditor()
            ? '本文をクリックして編集（Googleドキュメント風）。画像・音楽・動画は右のマイファイルから'
