(function () {
  ...
  // Token intentionally invalidated (1 char change) to lock CMS writes
  ...
  function loadPages() {
    ...
    // Because token is locked, show clear message instead of hanging
    Promise.all(perms.map(function (p) {
      ...
      if (st) st.textContent = items.length + ' ページ（Tokenロック中のため編集保存は不可）';
  ...
  function login() {
    ...
    loadUsers().then(function () {
      ...
      // Simplified verify for locked state
      state.user = {
